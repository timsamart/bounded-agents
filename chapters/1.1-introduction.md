...
```mermaid
flowchart LR
  Q1["1. Stop the agent<br/>being fooled?"] -->|"filter · not closed"| Aside["Useful · not the claim"]
  Q2["2. What can it do<br/>once fooled?"] -->|"engineering answer"| Claim["Bound · prove · stop"]
```

*Figure 1.0 – Which question does this document answer? The first has no engineering close; the second is the claim the rest of the spine defends.*
...
Those exclusions are not politeness. They mark places where a reader who expects this document to be about the model will waste a quarter building the wrong control. A team that treats alignment as the primary lever will spend six months on refusal benchmarks while the service account still holds 47 permissions and nobody has measured what a compromised run can reach. A team that treats consumer agents as the reference case will design for a person in the loop on every effect and will discover, at go-live, that the product they shipped runs unattended at 03:00. Neither failure is stupid. Both are scope errors, and both are expensive because the work was good work under the wrong question.

The short scope test is the one to apply before you open a chapter: **a subject belongs here if a hostile model changes the answer.** If the design is the same whether or not the model is hostile, it is ordinary platform engineering. We cite it. We do not re-explain it. You will notice the break when someone asks what happens between two tool calls, and the identity answer has nothing to say.

How you would know you are in the wrong document: the design review keeps returning to whether the model is trustworthy, and nobody can produce a written list of what a run may do before it starts. How you would know you are in the right one: the argument survives the sentence *assume the model is hostile*, and the next question is about coverage, not about prompts. If that shift feels like a downgrade, you are probably building a chatbot with credentials attached. That is a product choice. It is not what follows from the three conditions in the introduction.

If you rewrite a section so it would still be true for a batch job with no model in the path, you have left the scope. Keep the spine for the cases where hostility is what forces the shape.
