...
The moves compose because each one bounds a quantity the next one depends on. An architecture is the list of things a system has made impossible. A component list is the list of things it contains. The second is much easier to write, which is why there are so many of them.
