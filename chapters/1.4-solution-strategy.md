...
None of this makes the agent correct: a run that stays inside its envelope, records every effect, and stops on command can still post a wrong adjustment to the right claim, and the platform will prove faithfully that it did so. Containment is not judgement. A design review that treats the invariant set as a quality bar for the model's answers has misread every move.

None of it covers a path the platform does not mediate. The coverage fraction is the one figure here worth arguing about in your own setting rather than accepting from ours, because an unmediated connector is not a partial failure of move two. It is a place where none of the five moves apply. Publishing 94% does not make the other 6% theoretical; it makes them the adversary's shortlist.

None of it survives its own maintenance calendar being cut in a bad quarter. That is the only failure mode in this document with no alarm attached to it. Latency regressions page people. Coverage drops can be made to page people. Decay of drills, recertification, and the paved road does not page anyone unless you invent the page, which is why chapter 16 exists and why chapter 21 treats an unexercised control as already absent.

Finally, none of the five moves closes the residual set in chapter 21. Sequences of permitted calls, egress through allowed channels, a tired approver, and everything above the trusted computing base remain available to a competent adversary after every invariant holds. The moves buy a bound you can defend. They do not buy a boundary you can congratulate yourself for.
