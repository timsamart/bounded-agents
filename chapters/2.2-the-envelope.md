...
```mermaid
flowchart LR
  Role["Standing role<br/>union of every task"] --> Bad["Every run holds<br/>the maximum"]
  Need["Declared need"] --> Env["Per-run envelope<br/>intersection"]
  Reach["Principal reach"] --> Env
  Tier["Tier ceiling"] --> Env
  Env --> Bound["Run holds only<br/>what all three permit"]
```

*Figure 6.0 – Where does a run's authority come from? Not from the standing role's union, but from an intersection computed once at run start and never widened from inside the run.*
...
Treat the object as a contract fixed at one timestamp, not as live state the run can edit. Every mediated call carries a reference to this structure. The gateway compares the proposed operation and arguments against the `operations` array, decrements against `budget.tool_calls`, and refuses when `expires_at` has passed or the run identifier appears on the revocation set. None of that requires the agent to cooperate.

What the object deliberately omits is as load-bearing as what it includes. There is no field for widening, no runtime patch, no exception hook keyed to task description. The `derived_from` block exists so an incident review can answer *why this run held four operations* without opening three separate systems. A reviewer who sees `principal_reach.removed: 2` knows two declared operations never entered the envelope because Marta could not reach them.

How you would know the object had quietly stopped governing behaviour: calls succeed with operation names absent from the envelope but present in the role, which means derivation was bypassed or the gateway is not on the path. Or derived envelopes stop varying across principals while declared need in the manifest drifts upward. The JSON below is the reference shape. An example that does not validate against Appendix D is a defect in the document, not a suggestion.
