...
There is exactly one place in an agent run where a rule can attach, and it is not anywhere inside the model. It is the instant at which guessing becomes executing: the model proposes a structured call – an operation name and typed arguments, in a schema the model does not own – and something else decides whether that call runs. That instant is the seam. A shared tool protocol (today: the Model Context Protocol, MCP) puts the seam in the same place, in the same shape, across tools written by people who have never met. That is a precondition for governance, not governance itself. Adopting the protocol puts the rule-attachment point in a consistent place. It does not decide what happens there.

This chapter is about keeping mediation real at the place where calls physically happen (I1: no effect leaves except through a mediated call), and about one further property: every transition from guessing to executing passes through one interface whose schema the platform controls, and no authority is derived from anything the guessing side asserted about itself [A-6.4]. The second clause is the one that gets violated, by a design that reads a tool description and treats what it says as configuration.

```mermaid
flowchart LR
  Guess["Guessing<br/>model proposes"] --> Seam["Seam<br/>typed call"]
  Seam --> Decide["Platform decides<br/>permit · refuse · record"]
  Decide --> Do["Executing<br/>tool / system of record"]
```

*Figure 8.0 – Where can a rule attach? Only at the typed call: the instant guessing becomes a named operation the platform can permit, refuse and record.*
...
What it gives the platform is four things: a schema for describing an operation and for invoking it, which is what makes an allow-list of typed operations expressible at all; a discovery mechanism by which a client learns what a server offers; transports; and a shared result shape. Every statement in this chapter about what the protocol does and does not carry is a statement about one revision of MCP, named here with the date on which it was read [@mcp2025spec].

## What it does not carry

The protocol carries plumbing semantics and not authority semantics. The gap is six properties wide. Name them once, then say where the platform does the missing work.

1. **No per-call authority reference.** Nothing in a call names the authority under which it is made.
2. **No attenuation semantics.** Nothing expresses *this call, but weaker*.
3. **No possession binding of the caller.** Possession of whatever the transport carries remains sufficient.
4. **No declared side-effect class or idempotency contract.** Nothing distinguishes a read from a payment, or says whether repeating a call is safe.
5. **No signed manifest with provenance.** A server's self-description arrives with no publisher identity attached.
6. **No standard refusal shape.** A platform refusal, a tool failure, and a tool that succeeded and returned nothing arrive at the model as three flavours of one undifferentiated text.

The first three are about authority. The platform compensates for all three in one place: the gateway injects a reference to the envelope derived in chapter 6, the envelope is where attenuation lives, and sender constraint stops at the gateway's own edge, where chapter 5's run credential is verified. Compensating rather than carrying means these properties hold only inside one estate. That is why chapter 18 struggles with composition.

The fourth is about effects. A retry after a timeout is an ordinary engineering event, and the difference between a retried read and a retried adjustment is the difference between a log line and a phone call from Marta. The platform compensates with a registry field supplied by a person at onboarding.

The last two are about trust in what the far side says about itself. The platform answers the fifth with an internal registry and pinned digests, and the sixth with a refusal object of its own invention that works inside one estate and travels nowhere.
