...
First, the object the human approved has to be the object the system executes. That check happens at execution, not at approval time (I5: approved equals executed). You freeze the call and compare hashes.
...
sequenceDiagram
  ...
  participant S as Claims<br/>system of record
...
*Figure 9.1 – What makes a regenerated payload impossible to hide? The approval is bound to a digest rather than to an intention, so an object that is not the frozen call reaches execution with nothing attached to it.*
