...
```mermaid
flowchart LR
  EXT["Content from outside"]
  ING["Ingestion with<br/>provenance stamp"]
  MEM["Memory store<br/>quarantine tier"]
  RET["Retrieval under<br/>run principal"]
  EFF["Effect above the<br/>reversibility line"]
  EXT --> ING --> MEM --> RET --> EFF
  MEM -.->|"cannot justify alone"| EFF
```

*Figure 10.1 – Where does external content become actionable? Provenance and quarantine tier travel with the item; only non-quarantined justification can carry an effect above the line.*
...
*Figure 10.2 – At which point does something an outsider wrote become something the agent acts on? At the last solid edge, and only when something other than quarantined content justifies the effect; the dashed edge is the write path nobody mediated, and it is the one to go looking for.*
...
The Borealis handling note on 2026-07-14 is the case that looks like a permission and is not. Kai's text entered through a quarantined document, was summarised into memory keyed to the claimant, and surfaced in Marta's run as institutional knowledge. Nothing in that chain touched derivation, because derivation never reads memory. What failed was the sole-justification rule on effects above the reversibility line, not I8.

How you would know the separation had eroded: a derivation service gains a feature that "suggests" declared need from recent runs, or an incident review asks why declared need changed without a signed manifest diff. The audit for I8 is mechanical: list every input to derivation and show its provenance is manifest, entitlement service, or signed tier bundle. Memory appears on none of those lists.
