...
sequenceDiagram
  ...
  G->>E: durable record written
  E-->>G: acknowledged
  G->>S: effect permitted
```

*Figure 11.1 – What order makes the proof honest? The record exists before the effect is permitted; a path that writes afterwards can always produce an effect with no matching entry.*
...
*Figure 11.2 – How can one record be both unalterable and erased? Nothing in the chain changes when the key dies, so the sequence remains verifiable while the content it pointed at becomes permanently unreadable.*
...
*Four top-level objects answer the four questions, and `effect_state` is `declared` with `settled_by` still null, because this event was written and acknowledged before the adjustment reached the claims system of record [A-4.5].*
...
At 09:12 Marta's run proposes `tool:claims-core/post-adjustment` for €4,180 on `CLM-2026-448120`, inside a tier two envelope capped at €5,000 with a budget of 40 tool calls. The approval is already recorded. The gateway writes the declared-effect event, and the evidence store's primary is midway through a failover, so the durability acknowledgement does not return inside the 400 ms the gateway allows it. The call is refused with a retryable refusal, and the run retries. Two retries later, at 09:12:41 UTC, the write acknowledges, the adjustment posts to the claims system of record, and a settlement event links back to the declared one. Three of the 40 tool calls are gone, and Marta saw a delay of 37 s and no error. Now run the same failover with best-effort logging. The adjustment posts at 09:12:04, the log write dies on a queue that dies with its process, and what remains is a €4,180 movement in the claims system of record, correct in every respect, that nobody authorised as far as any record shows. None of that is visible at the time. It is visible on 2028-01-19, when the reply has to say that the platform holds no record of an adjustment the claims system holds, and every other record in the reply becomes less believable for standing next to it.
