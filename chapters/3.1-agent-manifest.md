...
The manifest is the signed composition chapter 12's lifecycle promotes. Every control that says *the version that ran* points here: envelope derivation reads `declared_need` and the policy reference; run credentials carry `digest`; quarantine marks this digest unpromotable; recertification diffs this object against what production last executed.

The fields are not equally volatile. `instructions.prompt_digest` changes on prose edits that never touch authority. `declared_need` should change rarely and only through review, because it is an I8 input. `promotion.evaluation` is the audit record for the weakest gate in the document: a number, a threshold, an override with an expiry, or a hard pass.

What fails if the object is wrong is specific. A digest mismatch between credential and registry means production is running an unsigned composition. A `declared_need` copied from the role restores the union with paperwork. An empty `delegation` array stops the platform from deriving a lawful child while a framework may still spawn. The JSON is bytes, not argument.
...
What fails first in practice is agreement between the registry and the runtime: a run that started on an older digest while the promoted pointer moved, or routing that serves a model string outside `allowed_models` because the table was edited without a manifest bump. The object is what makes that disagreement detectable rather than anecdotal.
