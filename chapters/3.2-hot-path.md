...
```mermaid
flowchart LR
  C["Mediated call"] --> D["Decision at<br/>enforcement point"]
  PB["Signed policy bundle<br/>local copy"] --> D
  EN["Entitlements<br/>local copy"] --> D
  RV["Revocation state<br/>local copy"] --> D
  PS["Central policy service<br/>not on the path"] -.->|"rejected"| D
```

*Figure 13.1 – Where is the decision evaluated? At the enforcement point against local copies; a synchronous round trip to a central service is the design this chapter refuses.*
...
*Figure 13.2 – When this permits a call, how old is the information it permitted it on? Every input edge carries the maximum age at p99 the platform accepts for that input, and the single edge without a number is fixed at run start rather than refreshed during the run.*
...
The failure mode for never-fired rules is slow and dignified. A deny rule written during an incident still sits in the bundle after the operation it names was retired in a migration nobody told policy about. When the operation returns under a new name, the rule does not fire either. The estate believes it is protected against something that no longer exists, and is not protected against something that does. A rule that fires on every call is either load-bearing or vestigial; Borealis found one of each in the first quarterly pass.

What an operator notices: refusal rates move while the bundle version stays fixed, or an incident review asks which rule refused a call and the answer is none despite a deny everyone remembers writing. The fix is a counter per rule id, a quarterly export, and deletion of rows that never fired after confirming the operation is gone.
