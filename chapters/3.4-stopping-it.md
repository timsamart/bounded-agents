...
The third leg of the claim in chapter 1 lands here: revocation takes effect within a stated interval, without the agent's cooperation (I6). That claim is falsified in this chapter or nowhere. There is no other place in the document where it can turn out to be false. Read the interval numbers below with more suspicion than the rest of the document invites.
...
*Figure 15.1 – When you pull one of these, what stops and what keeps running? Each switch cuts one layer and leaves the other four standing; the dashed path is the one no switch in this chapter reaches.*
...
What fails if the rule is treated as rhetoric is visible in the filing, not in the architecture diagram. Borealis listed egress cut in a supervisory pack alongside four exercised mechanisms while the production row stayed blank for seven months, because staging had a number and production did not. An incident commander who reaches for a switch that exists only in a runbook discovers the same thing Marta's platform lead discovered on 2026-03-02: the fast thing available is the wrong thing to do, and the measured interval was never measured. A reviewer who accepts *documented* as *present* will approve a control register that is half theatre. The honest register marks absent controls absent and leaves the interval column empty until a drill fills it.

How you would know the distinction is live in your estate: the quarterly read-out names a date and an interval for each switch, or it names absence. There is no third category. If your register has five rows and only four dates, the fifth row is the one Kai hopes you will forget when the destination matters.
