...
```mermaid
flowchart LR
  subgraph SURV["Survive the hop"]
    AT["Attenuation"]
    EV["Evidence chain"]
  end
  subgraph BREAK["Stop at the boundary"]
    AP["Approval binding"]
    IN["Principal intent"]
    BU["Shared budget<br/>unless contended"]
  end
  P["Parent run"] --> SURV
  P --> BREAK
  SURV --> C["Child run"]
...
*Figure 18.1 – Which properties cross an agent-to-agent call? Two survive by structure and arithmetic; three fail because the objects they were bound to do not exist on the far side.*
...
*Figure 18.2 – Which of the parent's properties reach the child? The envelope narrows along the mediated path and the evidence chain closes behind it, while the approval and the intent behind it stop at the boundary.*
...
A third number is worth plotting beside them: the fraction of runs in the estate that are children rather than roots. Its direction matters more than its value. A rising fraction means work is migrating into trees. Every property in this chapter that holds inside a tree holds because one operator owns the counter, the chain and the keys. When that fraction rises without a matching review of depth and fan-out, the estate is scaling composition faster than it is scaling the controls composition requires.

What failure looks like in practice is not a single catastrophic tree. It is a slow drift. On 2026-08-01 Borealis observed depth two on 11% of runs, up from 4% in May, while the configured depth limit had been 3 since launch and unchanged. Fan-out had not moved. The interesting signal was the child fraction, not the limit breach: nobody had raised the limit; teams had routed more work through declared children because the framework made it pleasant. Evidence volume had risen 40% on the same claim volume. The counter contention panel stayed green. The verification job did not. An operator who watches only the limits misses that story. An operator who watches the three numbers together sees composition becoming normal before the exfiltration-shaped tree arrives.

If the configured limits and the observed maxima converge and stay converged for two quarters, treat that as a warning, not as success. Limits that always bind are limits nobody is testing. The estate has probably stopped using composition for legitimate work and started routing around the graph declaration instead, which is chapter 7's decay curve wearing a different hat.
