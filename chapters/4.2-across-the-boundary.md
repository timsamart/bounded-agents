...
```mermaid
flowchart TB
  Own["What you still own"] --> I1["Possession-bound<br/>credential"]
  Own --> I2["Verifiable claim<br/>recipient can check"]
  Own --> I3["Contractual<br/>evidence obligation"]
  Lost["What mutual distrust<br/>removes"] --> U1["Shared envelope"]
  Lost --> U2["Audit of their run"]
  Lost --> U3["Shared policy domain"]
```

*Figure 19.0 – What survives the organisational line? Three instruments you can verify without trusting the peer; three assurances that currently require a trust neither side will grant.*
...
Three failure modes show up in the first bilateral incident review. The first is an attestation that arrived on schedule and covered the wrong fact: mediation coverage published as a percentage, when the dispute is about a single untyped egress path neither side listed. The second is an attestation that verified cryptographically and then sat unread for a quarter, because nobody owned the register the previous section asked for. The third is a reconstructed narrative assembled from logs neither side agreed were authoritative, offered to counsel as if it were evidence. Each mode is cheaper to prevent with chapter 11's write-before-effect discipline on your own side than to litigate after the effect has already landed in a foreign system of record.

What this section does not do is allocate liability. Counsel does that under the contract and the applicable law. What it does is remove the pretence that a shared logo, a vendor datasheet, or a slide titled "federated trust" is a substitute for bytes one side can still produce when the other side's story has gone quiet.
...
Which counterparty attestations have expired, and are the corresponding integrations still live? The attestation register answers both: each bilateral schedule carries an expiry date, a named owner, and the audiences or integrations it governs. When a date passes, the owner runs the same calendar query used for recertification in chapter 15 – list lapsed schedules, then list audiences still accepting their credentials – and disables or demotes the integration per the posture agreed before launch, not after the first post-expiry call. Call logs alone will not surface that gap; the register and its owner will [ADR-30].
